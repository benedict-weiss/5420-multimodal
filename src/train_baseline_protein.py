"""
train_baseline_protein.py — Protein-only baseline training (Model 1 variant)

Same structure as train_baseline.py but swaps the RNA PCA branch for CLR-normalized
protein features. Protein is already low-dimensional (134 markers), so no PCA is
applied and there is no train/test fit step to leak between splits.

1. Load preprocessed data:
    - Protein CLR matrix (134-d) and labels from preprocessing.py
    - Split by donor into train/test

2. Build model:
    - MLPEncoder(input_dim=134, hidden_dim=256, output_dim=128) — no L2 normalization
...
5. Command-line usage:
    - python src/train_baseline_protein.py --data_path data/ --seed 42 --epochs 50
"""
...
def make_protein_loader(
    protein: np.ndarray, labels: np.ndarray, batch_size: int, shuffle: bool
) -> DataLoader:
    dataset = TensorDataset(
        torch.as_tensor(np.asarray(protein, dtype=np.float32)),
...
def evaluate_epoch(
    ...
    for protein, y in loader:
        protein, y = protein.to(device), y.to(device)
        logits = classifier(encoder(protein))
...
def main(args: argparse.Namespace) -> None:
    ...
    # Protein-only: discard RNA modality
    _, protein_adata = split_modalities(adata)

    # CLR is per-cell, so there's no fit/transform leakage concern — normalize each split independently
    train_protein = protein_adata[train_global_idx].copy()
    test_protein = protein_adata[test_global_idx].copy()
    train_protein_clr = preprocess_protein(train_protein)
    test_protein_clr = preprocess_protein(test_protein)

    train_labels = labels_all[train_global_idx]
    test_labels = labels_all[test_global_idx]

    print(
        f"Prepared matrices: train protein {train_protein_clr.shape}, "
        f"test protein {test_protein_clr.shape}"
    )

    # Carve out a validation split for checkpoint selection
    ...
    train_loader = make_protein_loader(
        train_protein_clr[train_local_idx], train_labels[train_local_idx],
        batch_size=args.batch_size, shuffle=True,
    )
    val_loader = make_protein_loader(
        train_protein_clr[val_local_idx], train_labels[val_local_idx],
        batch_size=args.batch_size, shuffle=False,
    )
    test_loader = make_protein_loader(
        test_protein_clr, test_labels, batch_size=args.batch_size, shuffle=False,
    )

    # 2. Build model — input_dim is the number of protein markers (134 for BMMC dataset)
    encoder = MLPEncoder(
        input_dim=train_protein_clr.shape[1],
    ...
    print("\n=== Training protein-only baseline ===")
    for epoch in range(1, args.epochs + 1):
        ...
        for protein, y in train_loader:
            protein, y = protein.to(device), y.to(device)
            optimizer.zero_grad()
            loss = F.cross_entropy(classifier(encoder(protein)), y)
            loss.backward()
            optimizer.step()
    ...
    run_dir = Path(args.output_dir) / f"baseline_protein_seed{args.seed}_{run_ts}"
    ...
    # Persist the protein marker list for feature alignment on reuse
    protein_marker_names = list(protein_adata.var_names)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train protein-only MLP baseline")
